...
export const Login = () => {
  const handleGoogleLogin = () => {
    ...
    window.google.accounts.id.initialize({
      client_id: '334274422521-q0ulboehavlpf6njb0dblpaj83uvajr0.apps.googleusercontent.com',
  ...
  const handleCredentialResponse = async (response) => {
    // Send token to the server to verify and exchange for user data
    const idToken = response.credential;
    try {
      const serverResponse = await fetch('http://localhost:5173/auth/google', {
        method: 'POST',
        headers: {
